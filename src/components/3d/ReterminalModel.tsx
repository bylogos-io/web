...
export function ReterminalModel(props: any) {
    ...
    useFrame((state) => {
        ...
        group.current.scale.setScalar(THREE.MathUtils.lerp(group.current.scale.x, targetScale, ANIM_CONFIG.lerpSpeed));
        ...
        group.current.position.z = THREE.MathUtils.lerp(group.current.position.z, targetZ, 0.03);

        /**
         * 3. LÓGICA DE GIRO
         */
        // Usa la configuración finalRotationY cuando está activo
        const baseRotationY = active ? ANIM_CONFIG.finalRotationY : ANIM_CONFIG.initialRotation;
    ...
    return (
        <group ref={group} {...props} dispose={null} scale={0}>
            <Float speed={1.5} rotationIntensity={0.2} floatIntensity={ANIM_CONFIG.floatIntensity}>
                <group
